...
if qp.get("view") == "jobs":
    ...
    st.markdown("### What type of jobs are you looking for?")
    
    with st.form("job_search_form", clear_on_submit=False):
        col1, col2 = st.columns(2)
        
        with col1:
            job_title = st.text_input("Job title or role", placeholder="e.g., Software Engineer, Marketing Manager")
            location = st.text_input("Location", placeholder="e.g., San Francisco, Remote, New York")
        
        with col2:
            company = st.text_input("Company (optional)", placeholder="e.g., Google, Microsoft")
            skills = st.text_input("Key skills (optional)", placeholder="e.g., Python, React, Marketing")
        
        search_submitted = st.form_submit_button("Find Jobs", use_container_width=True)
    
    if search_submitted:
        # For now, show mock results - we'll integrate real APIs later
        st.markdown("---")
        ...
        mock_jobs = [
            ...
            },
            {
                ...
                "location": "Seattle, WA",
                "type": "Full-time",
                "description": "We're seeking a Software Engineer II to work on our core platform...",
                "requirements": ["Java", "Spring", "Microservices", "3+ years experience"],
                "match_score": 68
            }
        ]
        ...
        for i, job in enumerate(mock_jobs):
            with st.container():
                ...
                with col1:
                    ...
                    st.markdown(f"*{job['company']} • {job['location']} • {job['type']}*")
                    st.markdown(f"Match: {job['match_score']}%")
                
                with col2:
                    if st.button(f"Analyze", key=f"analyze_{i}"):
                        st.session_state["selected_job"] = job
                        st.session_state["stay_on"] = "Job Analysis"
                        st.rerun()
                
                with col3:
                    if st.button(f"Apply Now", key=f"apply_{i}"):
                        st.session_state["selected_job"] = job
                        st.session_state["stay_on"] = "Ready to Apply"
                        st.rerun()
                
                st.markdown("---")
    
    # Back to main app
    if st.button("← Back to Resume Tailoring"):
...
with col1:
    if st.button("🔍 Explore Jobs", use_container_width=True):
        st.query_params["view"] = "jobs"
        st.rerun()
with col2:
    st.markdown("<div style='text-align:center; color: var(--ink-600); font-size: 13px;'>or</div>", unsafe_allow_html=True)
with col3:
    st.markdown("<div style='text-align:center; color: var(--ink-600); font-size: 13px;'>Tailor existing resume</div>", unsafe_allow_html=True)
...
if selected_job:
    st.session_state["job_analysis_mode"] = True
    
    # Job Analysis Header
    st.markdown(f"### 📊 Analyzing: {selected_job['title']}")
    st.markdown(f"**{selected_job['company']}** • {selected_job['location']} • {selected_job['type']}")
    st.markdown(f"*Match Score: {selected_job['match_score']}%*")
    
    st.markdown("---")
    
    # Two paths: Ready to Apply vs Need to Prepare
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("#### 🚀 Ready to Apply?")
        st.markdown("If you feel confident about this role, let's tailor your application materials:")
        if st.button("Tailor Resume & Cover Letter", use_container_width=True):
            # Pre-fill the job description
            st.session_state["pasted_job"] = f"""
{selected_job['title']} at {selected_job['company']}

Location: {selected_job['location']}
Type: {selected_job['type']}

Description:
{selected_job['description']}

Requirements:
{chr(10).join(f"• {req}" for req in selected_job['requirements'])}
"""
            st.session_state["selected_job"] = None
            st.session_state["job_analysis_mode"] = False
            st.rerun()
    
    with col2:
        st.markdown("#### 🎯 Need to Prepare First?")
        st.markdown("Get personalized steps to become a stronger candidate:")
        if st.button("Get Action Plan", use_container_width=True):
            # This will trigger the better-candidate analysis
            st.session_state["pasted_job"] = f"""
{selected_job['title']} at {selected_job['company']}
...
st.session_state.setdefault("job_analysis_mode", False)
